...
//  OS lab 18
//
//  Created by YASH VINAYVANSHI on 31/05/21.
//
...
int main(){
    ...
    int service_sequence[no_of_requests+2];
    int head_movement[no_of_requests+2];
    ...
    //check from where to serve the request sequenc
    int index=-1;
    for(int i=0; i<no_of_requests; i++){
    ...
    if(request_sequence[no_of_requests-1]-init_head_pos>init_head_pos){
        ...
        //in LOOK only this part of the code will be removed
        service_sequence[count]=0;
        head_movement[count]=service_sequence[count-1]-0;
        current_head_pos=0;
        total_head_movement+=head_movement[count];
        count++;
        service_sequence[count]=199;
        head_movement[count]=abs(service_sequence[count-1]-199);
        current_head_pos=199;
        total_head_movement+=head_movement[count];
        count++;
        for(int i=no_of_requests-1; i>=index+1; i--){
            service_sequence[count]=request_sequence[i];
            head_movement[count]=abs(current_head_pos-request_sequence[i]);
            total_head_movement+=head_movement[count];
            current_head_pos=request_sequence[i];
        ...
        //assumed that there are maximum 200 tracks
        //go to the end
        //in LOOK only this part of code will be removed
        int max=199;
        service_sequence[count]=max;
        head_movement[count]=max-service_sequence[count-1];
        current_head_pos=max;
        total_head_movement+=head_movement[count];
        count++;
        service_sequence[count]=0;
        head_movement[count]=service_sequence[count-1]-0;
    ...
    for(int i=1; i<no_of_requests+2; i++){
